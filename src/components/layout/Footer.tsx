      ...
      <div className="max-w-7xl mx-auto px-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-16 mb-20">
          <div className="md:col-span-5 space-y-8">
            <div className="text-3xl font-extrabold text-on-surface tracking-tight">35utech<span className="text-primary">.</span></div>
            <p className="text-on-surface-variant text-base leading-relaxed max-w-sm font-medium">
              Merekayasa solusi digital berkinerja tinggi untuk generasi perusahaan global berikutnya.
            </p>
            <div className="flex gap-4">
              {SOCIALS.map((social, idx) => (
                <Link key={idx} href={social.href} aria-label={social.label} className="w-11 h-11 rounded-xl border border-outline/50 hover:border-primary hover:text-primary hover:bg-primary/5 flex items-center justify-center transition-all duration-300 text-on-surface-variant">
                  {social.icon}
                </Link>
              ))}
            </div>
          ...
          <div className="md:col-span-7 grid grid-cols-2 md:grid-cols-3 gap-10">
            <div>
              ...
              <ul className="space-y-5 text-on-surface-variant text-sm font-medium">
                <li><Link className="hover:text-primary transition-colors" href="#">Rekayasa Perangkat Lunak</Link></li>
                <li><Link className="hover:text-primary transition-colors" href="#">Desain Pengalaman</Link></li>
                <li><Link className="hover:text-primary transition-colors" href="#">Akselerasi Bisnis</Link></li>
              </ul>
            </div>
            <div>
              <h4 className="text-on-surface font-bold text-xs uppercase tracking-[0.2em] mb-8">Perusahaan</h4>
              <ul className="space-y-5 text-on-surface-variant text-sm font-medium">
                <li><Link className="hover:text-primary transition-colors" href="#portofolio">Portofolio</Link></li>
                <li><Link className="hover:text-primary transition-colors" href="#layanan">Layanan</Link></li>
                <li><Link className="hover:text-primary transition-colors" href="#kontak">Hubungi Kami</Link></li>
              </ul>
            </div>
            <div className="col-span-2 md:col-span-1">
              ...
              <div className="relative group">
                <input
                  ...
                  className="w-full bg-surface-variant border border-outline/50 rounded-xl px-5 py-3.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                />
                <button className="absolute right-1.5 top-1/2 -translate-y-1/2 w-9 h-9 bg-primary text-white rounded-lg flex items-center justify-center hover:bg-primary/90 transition-all">
        ...
        <div className="pt-8 border-t border-outline/30 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-on-surface-variant text-sm font-medium">© 2026 35utech</p>
